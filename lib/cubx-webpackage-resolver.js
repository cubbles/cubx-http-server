...
const cubxWebpackageResolver = {
  ...
  resolve: function (req, res) {
    const Url = url.parse(req.url);// eslint-disable-line node/no-deprecated-api
    ...
    const webpackageId = Url.pathname.split('/')[1];
    let rewrittenWebpackageId;
    let exists = true;
    ...
    if (exists && needsRewrite) {
      const rewrite = req.url.replace(webpackageId, rewrittenWebpackageId);
      req.url = rewrite;
    }
